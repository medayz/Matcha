...
export default class EditInfos extends Component {
  ...
  onChangeTags = async (newValue) => {
    if (newValue) {
      ...
      addTags(this.state.username, Tags, head).then(({
        data
      }) => {
        this.calluserTags();
        this.callTags();
      }).catch(err => console.log('layn3altabonmok!'));
    }
  };
  clearErrorState = () => {
    this.setState({
  ...
  callTags = async () => {
    await getAllTags().then(({
      ...
      let filteredTags = this.state.options.filter(itemX => !yFilter.includes(itemX.value));
      this.setState({
  ...
  async componentWillMount() {
    ...
    await axios.get(`http://localhost:1337/api/users/get/${user}`).then(res => {
      if (res.data.data.props) {
        ...
        if (user.fName) this.setState({
          fName: user.fName
        });
        if (user.lName) this.setState({
          lName: user.lName
        });
        if (user.gender) this.setState({
          gender: user.gender
        });
        if (user.location) this.setState({
          location: user.location
        });
        if (user.bio) this.setState({
          bio: user.bio
        });
        if (user.sexualPref) this.setState({
          sexualPref: user.sexualPref
        });
        if (user.birthDate) this.setState({
