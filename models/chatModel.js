...
module.exports = {
    getUserChats: async username => {
        return await query.getAllSpecialNodes(
            "MATCH (:User {username: $name})-[]->(c:Chat)<-[]-(u:User) RETURN collect({date: c.dateLastMsg, time: c.timeLastMsg, name: u.username})",
            {
                name: username
            }
        );
    ...
    deleteChat: (user1, user2) => {
        query.execute(
            "MATCH (:User {username: $name1})-[:PARTICIPATE_IN]->(c)<-[:PARTICIPATE_IN]-(:User {username: $name2}) DETACH DELETE c;",
            {
                name1: user1,
                name2: user2
            }
